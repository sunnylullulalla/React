...
function ComingSoon() {
  ...
  const bigMovieMatch = useRouteMatch<{ movieId: string }>(
    "/comingSoon/movies/:movieId"
  );
  const { data } = useQuery(["movie", "ComingSoon"], getComingSoon);
  const onOverlayClick = () => history.push("/comingSoon");
  ...
  const onCardClicked = async (movieId: number) => {
    history.push(`/comingSoon/movies/${movieId}`);
